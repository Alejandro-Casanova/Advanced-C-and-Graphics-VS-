...
const b2Vec2 GRAVITY(0.0f, -40.0);
...
void EditorScreen::drawUI()
{
    ...
    if (m_selectMode == SelectionMode::PLACE && !isMouseInUI()) {
        ...
        if (m_objectMode == ObjectMode::PLATFORM) {
            ...
            tmpLight.color = Bengine::ColorRGBA8((GLubyte)m_currentColorPickerRed, (GLubyte)m_currentColorPickerGreen, (GLubyte)m_currentColorPickerBlue, (GLubyte)m_currentColorPickerAlpha);
    ...
    m_spriteBatch.begin();
    {//Draw color picker quad and codes
        ...
        destRect.y = m_bSlider->getYPosition().d_scale * m_window->getScreenHeight() + m_window->getScreenHeight() / 2.0f -
        ...
        m_spriteBatch.draw(destRect, glm::vec4(0.0f, 0.0f, 1.0f, 1.0f), m_blankTexture.id, 0.0f, Bengine::ColorRGBA8((GLubyte)m_currentColorPickerRed, (GLubyte)m_currentColorPickerGreen, (GLubyte)m_currentColorPickerBlue, 255));

        //Get color code
        GLuint colorCode = 0;
        colorCode |= (GLuint)m_currentColorPickerRed << 2 * 8 * sizeof(GLubyte);
        colorCode |= (GLuint)m_currentColorPickerGreen << 1 * 8 * sizeof(GLubyte);
        colorCode |= (GLuint)m_currentColorPickerBlue;

        //Format color code
        std::stringstream buffer;
        buffer << "0x" << std::setfill('0') << std::right << std::setw(6) << std::hex << colorCode << std::endl;

        m_spriteFont.draw(m_spriteBatch, buffer.str().c_str(),
            glm::vec2(destRect.x + QUAD_SIZE / 2, destRect.y + QUAD_SIZE + 2.5f),
            glm::vec2(0.5f), 0.0f, Bengine::ColorRGBA8(0, 176, 160, 255), Bengine::Justification::MIDDLE);
 
        buffer.str(std::string());//Clear buffer
        buffer << "B: " << std::dec << (int)m_currentColorPickerBlue << std::endl << "G: " << (int)m_currentColorPickerGreen << std::endl << "R: " << (int)m_currentColorPickerRed;

        m_spriteFont.draw(m_spriteBatch, buffer.str().c_str(),
            glm::vec2(destRect.x + QUAD_SIZE + 5.0f, destRect.y),
            glm::vec2(0.5f), 0.0f, Bengine::ColorRGBA8(0, 176, 160, 255), Bengine::Justification::LEFT);

    }

    //{//Draw radio button labels
    //    glm::vec2 pos;
    //    pos.x = m_rigidRadioButton->getXPosition().d_scale * m_window->getScreenWidth() - m_window->getScreenWidth() / 2.0f + m_rigidRadioButton->getWidth().d_offset / 2.0f;
    //    pos.y = -m_rigidRadioButton->getYPosition().d_scale * m_window->getScreenHeight() + m_window->getScreenHeight() / 2.0f;
    //    m_spriteFont.draw(m_spriteBatch, "Rigid", pos, glm::vec2(0.5f), 0.0f, Bengine::ColorRGBA8(0, 176, 160, 255), Bengine::Justification::MIDDLE);

    //    pos.x = m_dynamicRadioButton->getXPosition().d_scale * m_window->getScreenWidth() - m_window->getScreenWidth() / 2.0f + m_dynamicRadioButton->getWidth().d_offset / 2.0f;
    //    pos.y = -m_dynamicRadioButton->getYPosition().d_scale * m_window->getScreenHeight() + m_window->getScreenHeight() / 2.0f;
    //    m_spriteFont.draw(m_spriteBatch, "Dynamic", pos, glm::vec2(0.5f), 0.0f, Bengine::ColorRGBA8(0, 176, 160, 255), Bengine::Justification::MIDDLE);
...
void EditorScreen::initUI()
{
    ...
    const float GROUP_BOX_WIDTH = 0.22f, GROUP_BOX_HEIGHT = 0.72f;
    // Add group box back panel
    m_groupBox = static_cast<CEGUI::GroupBox*>(m_gui.createWidget("TaharezLook/GroupBox", glm::vec4(0.001f, 0.0f, GROUP_BOX_WIDTH, GROUP_BOX_HEIGHT), glm::vec4(0.0f), "GroupBox"));
    ...
    {//Add Color picker
        ...
        const float X_POS = 0.01f, Y_POS = 0.05f;
        const float PADDING = 0.005f;

        m_rSlider = static_cast<CEGUI::Slider*>(m_gui.createWidget("TaharezLook/Slider", glm::vec4(X_POS, Y_POS, X_DIM, Y_DIM), glm::vec4(0.0f), "Red Slider"));
        m_rSlider->setMaxValue(255.0f);
        m_rSlider->setCurrentValue(m_currentColorPickerRed);
        ...
        m_gSlider = static_cast<CEGUI::Slider*>(m_gui.createWidget("TaharezLook/Slider", glm::vec4(X_POS + X_DIM + PADDING, Y_POS, X_DIM, Y_DIM), glm::vec4(0.0f), "Green Slider"));
        m_gSlider->setMaxValue(255.0f);
        m_gSlider->setCurrentValue(m_currentColorPickerGreen);
        ...
        m_bSlider = static_cast<CEGUI::Slider*>(m_gui.createWidget("TaharezLook/Slider", glm::vec4(X_POS + (X_DIM + PADDING) * 2, Y_POS, X_DIM, Y_DIM), glm::vec4(0.0f), "Blue Slider"));
        m_bSlider->setMaxValue(255.0f);
        m_bSlider->setCurrentValue(m_currentColorPickerBlue);
        ...
        m_aSlider->setCurrentValue(m_currentColorPickerAlpha);
    ...
    { // Add Object type radio buttons
        ...
        const float DIMS_PIXELS = 10.0f;
        const float PADDING = 0.052f;
    ...
    {//Add physics mode radio buttons
        const float DIMS_PIXELS = 10.0f;
        const float X_POS = 0.02f, Y_POS = 0.28f;
        ...
        // Rotation spinner
        m_rotationSpinner = static_cast<CEGUI::Spinner*>(m_gui.createWidget("TaharezLook/Spinner", glm::vec4(X_POS + PADDING * 3.0, Y_POS, 0.0f, 0.0f), glm::vec4(0.0f, 0.0f, DIMS_PIXELS * 5, DIMS_PIXELS * 2), "RotationSpinner"));
        ...
        // Light size spinner
        m_sizeSpinner = static_cast<CEGUI::Spinner*>(m_gui.createWidget("TaharezLook/Spinner", glm::vec4(X_POS + PADDING * 3.0, Y_POS, 0.0f, 0.0f), glm::vec4(0.0f, 0.0f, DIMS_PIXELS * 5, DIMS_PIXELS * 2), "SizeSpinner"));
    ...
    { // Add platform dimension spinners
        ...
        const float PADDING = 0.045f;
        const float TEXT_SCALE = 0.4f;
        m_widthSpinner = static_cast<CEGUI::Spinner*>(m_gui.createWidget("TaharezLook/Spinner", glm::vec4(X_POS, Y_POS, 0.0f, 0.0f), glm::vec4(0.0f, 0.0f, DIMS_PIXELS * 4, DIMS_PIXELS * 2), "WidthSpinner"));
        ...
        m_heightSpinner = static_cast<CEGUI::Spinner*>(m_gui.createWidget("TaharezLook/Spinner", glm::vec4(X_POS + PADDING * 2.0, Y_POS, 0.0f, 0.0f), glm::vec4(0.0f, 0.0f, DIMS_PIXELS * 4, DIMS_PIXELS * 2), "HeightSpinner"));
    ...
    { // Add save and back buttons
        const float X_DIMS = 0.1f, Y_DIMS = 0.05f;
        const float Y_POS = 0.5f;
        const float PADDING = 0.07f;
        m_saveButton = static_cast<CEGUI::PushButton*>(m_gui.createWidget("TaharezLook/Button", glm::vec4(GROUP_BOX_WIDTH / 2.0f - X_DIMS / 2.0f, Y_POS, X_DIMS, Y_DIMS), glm::vec4(0.0f), "SaveButton"));
        m_saveButton->setText("Save");
        m_saveButton->subscribeEvent(CEGUI::PushButton::EventClicked, CEGUI::Event::Subscriber(&EditorScreen::onSaveMouseClick, this));

        m_loadButton = static_cast<CEGUI::PushButton*>(m_gui.createWidget("TaharezLook/Button", glm::vec4(GROUP_BOX_WIDTH / 2.0f - X_DIMS / 2.0f, Y_POS + PADDING, X_DIMS, Y_DIMS), glm::vec4(0.0f), "LoadButton"));
        ...
        m_backButton = static_cast<CEGUI::PushButton*>(m_gui.createWidget("TaharezLook/Button", glm::vec4(GROUP_BOX_WIDTH / 2.0f - X_DIMS / 2.0f, Y_POS + PADDING * 2, X_DIMS, Y_DIMS), glm::vec4(0.0f), "BackButton"));
    ...
    case SDL_BUTTON_LEFT:
        ...
        if (m_selectMode == SelectionMode::SELECT) {
            ...
            if (m_selectedLight != NO_LIGHT) {
                ...
                m_currentColorPickerRed = m_lights[m_selectedLight].color.r;
                m_currentColorPickerGreen = m_lights[m_selectedLight].color.g;
                m_currentColorPickerBlue = m_lights[m_selectedLight].color.b;
                m_currentColorPickerAlpha = m_lights[m_selectedLight].color.a;
            ...
            if (m_selectedBox != NO_BOX) {
                ...
                m_currentColorPickerRed = m_boxes[m_selectedBox].getColor().r;
                m_currentColorPickerGreen = m_boxes[m_selectedBox].getColor().g;
                m_currentColorPickerBlue = m_boxes[m_selectedBox].getColor().b;
                // Set widget values
            ...
            case ObjectMode::PLATFORM:
                ...
                newBox.init(m_world.get(), pos, m_boxDims, texture, Bengine::ColorRGBA8((GLubyte)m_currentColorPickerRed, (GLubyte)m_currentColorPickerGreen, (GLubyte)m_currentColorPickerBlue, 255),
            ...
            case ObjectMode::PLAYER:
                ...
                // Re-init the player
                pos = m_camera.convertScreenToWorld(glm::vec2(evnt.button.x, evnt.button.y));
                m_player.init(m_world.get(), pos, glm::vec2(2.0f), glm::vec2(1.0f, 1.8f), Bengine::ColorRGBA8((GLubyte)m_currentColorPickerRed, (GLubyte)m_currentColorPickerGreen, (GLubyte)m_currentColorPickerBlue, 255));
                m_hasPlayer = true;
                break;
            case ObjectMode::LIGHT:
                ...
                newLight.color = Bengine::ColorRGBA8((GLubyte)m_currentColorPickerRed, (GLubyte)m_currentColorPickerGreen, (GLubyte)m_currentColorPickerBlue, (GLubyte)m_currentColorPickerAlpha);
...
void EditorScreen::updateMouseMotion(const SDL_Event& evnt)
{
    ...
    if (m_mouseButtons[MOUSE_RIGHT]) {
        m_camera.offsetPosition(glm::vec2(-evnt.motion.xrel / m_camera.getScale(), evnt.motion.yrel / m_camera.getScale()));
    }
...
void EditorScreen::refreshSelectedBox(const glm::vec2& newPosition)
{
    ...
    newBox.init(m_world.get(), newPosition, m_boxDims, texture, Bengine::ColorRGBA8((GLubyte)m_currentColorPickerRed, (GLubyte)m_currentColorPickerGreen, (GLubyte)m_currentColorPickerBlue, 255),
        false, m_physicsMode == PhysicsMode::DYNAMIC, m_rotation, uvRect);
...
{
    m_currentColorPickerRed = m_rSlider->getCurrentValue();
    refreshSelectedBox();
...
{
    m_currentColorPickerGreen = m_gSlider->getCurrentValue();
    refreshSelectedBox();
    refreshSelectedLight();
    return true;
}
...
{
    m_currentColorPickerBlue = m_bSlider->getCurrentValue();
    refreshSelectedBox();
    refreshSelectedLight();
    return true;
}
...
{
    m_currentColorPickerAlpha = m_aSlider->getCurrentValue();
    refreshSelectedBox();
    refreshSelectedLight();
    return true;
}
