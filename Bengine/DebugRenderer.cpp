...
namespace{///< Prevents collition with other "VERT_SRC" or "FRAG_SRC"
...
///Function outside namespace Bengine
...
void DebugRenderer::drawCircle(const glm::vec2& center, const Bengine::ColorRGBA8& color, float radius){
    ...
    auto start = m_verts.size();
    m_verts.resize(m_verts.size() + NUM_VERTS);
    for(int i = 0; i < NUM_VERTS; i++){
        float angle = (float)i / NUM_VERTS * 2.0f * M_PI;
        m_verts[start + i].position.x = center.x + cos(angle) * radius;
        m_verts[start + i].position.y = center.y + sin(angle) * radius;
...
void DebugRenderer::render(const glm::mat4& projectionMatrix, float lineWidth){
    ...
    GLuint pUniform = m_program.getUniformLocation("P");
